...
Esto genera `reporte_generado.xlsx` a partir de `datos_ejemplo.csv`.
...
```python
GROUP_BY_COLUMN = "producto"   # columna para agrupar
VALUE_COLUMN = "ventas"        # columna numérica a sumar
```

Y reemplaza `datos_ejemplo.csv` por tu propio archivo (mismo formato
de columnas, o ajusta el código a tus columnas reales).
